...
# This Python file will calculate a solution of a first-order differential equation
# using Euler's Method for ordinary differential equations with a given initial condition
# and step size
...
def main():
    ...
    while loop:
        ...
        mathOp = {'.': '.', '*': '*', '+': '+', '-': '-', '/': '/', "pi": "math.pi", "**": "**", "^": "**",
                  "log": "math.log", "log10": "math.log10", "log2": "math.log2", 'e': "math.e",
        ...
        try:
            ...
            y0 = float(input("Enter the initial condition value y(%d) = " % x0))
            y_t = float(input("Enter the end condition y(_): "))

            if y_t > x0:
                eq = str(input("Enter the function f(x,y). Use x and y as your variables\n"
                               "available symbols:[*, /, +, -, sqrt, **, pi, log, e, sin, cos, tan, asin, acos, "
                               "atan] : "))
                ...
                try:
                    ...
                    print("Answer:\n\ty(%s) = %s" % ("{:.1f}".format(y_t), "{:.5f}".format(y_ti)))
                ...
                print("The end condition must be greater than the initial condition x0: %d \n" % x0)
                loop = True
...
# Use Euler's method using the given step size and initial condition to find solution
...
def print_intro(stepSize, x0, y0, eq):
    ...
    print("\nEuler Method: \n\tx_i+1 = x_i + h\n\ty_i+1 = y_i + h * f(x_i, yi)\n")
    print("Given: \n\th = %s, x_0 = %s, y_0 = %s, and f(x,y) = %s\n" % ("{:.2f}".format(stepSize),
                                                                        "{:.1f}".format(x0),
                                                                        "{:.1f}".format(y0),
                                                                        function_p))
    print("Solution:")
...
def print_step(step, x_i, stepSize, y_old, y_ti):
    print("Step %d\n\tx_%d = x_%d + h = %s + %s = %s"
          % (step, step, step - 1, "{:.2f}".format(x_i), "{:.2f}".format(stepSize),
             "{:.2f}".format(x_i + stepSize)))
    print("\n\ty_x%d = y_x%d + h * f(x_%d, y_x%d) \n\t    = %s + %s * f(%s, %s) = %s\n"
          % (step, step - 1, step - 1, step - 1, "{:.2f}".format(y_old), "{:.2f}".format(stepSize),
             "{:.2f}".format(x_i), "{:.2f}".format(y_old), "{:.4f}".format(y_ti)))
