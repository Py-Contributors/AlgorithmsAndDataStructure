...
def Dijkstras(graph, source, destination):
    ...
    while not len(needsProcessing) == 0:
        ...
        for node in needsProcessing:
            if(shortestDistance[closest] > shortestDistance[node]):
                closest = node
        ...
        for neighbor in graph.adjList[closest].keys():
            ...
            if(shortestDistance[neighbor] > shortestDistance[closest] + edgeWeight):
                shortestDistance[neighbor] = shortestDistance[closest] + edgeWeight
