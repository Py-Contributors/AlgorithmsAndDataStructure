...
# Jump search is another searching algorithm, which is faster than linear search, but slower than binary search. Its complexity is O(squareroot(n)).
# Note : The array must be sorted in order fr it to work

from math import sqrt
  
def jumpSearch(arr:list , x:int , n:int): 
